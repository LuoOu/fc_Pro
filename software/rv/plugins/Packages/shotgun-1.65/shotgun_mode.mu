...
\: clearViews (void; )
{
    for_each (v; viewNodes()) setNodeInputs (v, string[]{});
    for_each (v; viewNodes()) try { deleteNode(v); } catch(...) {;}
}
...
//
{
    ...
    class: Prefs
    {
        ...
        method: writePrefs (void; )
        {
            ...
            writeSetting ("Shotgun", "autoStream", SettingsValue.Bool(autoStream));
        ...
        method: readPrefs (void; )
        {
            ...
            let SettingsValue.Bool b6 = readSetting ("Shotgun", "autoStream",
                    SettingsValue.Bool(true));
            autoStream = b6;
    ...
    method: toggleStreaming (void; Event e)
    {
        this._prefs.autoStream = !this._prefs.autoStream;
        _prefs.writePrefs();
    }
    ...
    method: autoStreaming (int; )
    {
        deb ("autoStreaming pref is %s\n" % this._prefs.autoStream);
        if (this._prefs.autoStream == true) then CheckedMenuState else UncheckedMenuState; 
    }
    ...
    */
    ...
    {
        // we dont really have a great way to validate a URL
        // at this point, so we just assume true for all strings
        // that start with http, and leave it to ffmpeg to 
        // handle the error
        if (regex("^http").match(m)) 
        {
            deb ("Assuming %s is a Streaming type\n" % (m));
            return true;
        }
    ...
    method: mediaTypeFallback (string; string mediaType, StringMap info, bool autoStream)
    {
        if (shotgun_fields.mediaTypePathEmpty (mediaType, info))
        {
            ...
            if (newType != "")
            {
                print ("INFO: no '%s' media, switching to '%s'\n" % (_prefs.loadMedia, newType)); 
                mediaType = newType;
            }
            else 
            {
                extra_commands.displayFeedback("Unable to load '%s'" % mediaType);
                print ("WARNING: Version %s has no local media. See Version details here: %s/detail/Version/%s\n" % (info.find("id"), _shotgunState._serverURL, info.find("id")));

                // try streaming 
                if (autoStream)
                {
                    print ("INFO: trying streaming from shotgun server.\n");
                    return "Streaming";
                } else {
                    print ("WARNING: auto streaming is OFF. Returning media type: %s\n" % mediaType);
                }
            }
        }
        return mediaType;
    ...
    method: swapMediaFromInfo (void; string mediaType, string sourceName)
    {
        ...
        {
            // just skip
            // deb ("WARNING: source %d has no version ID\n" % sourceName);
            return;
        }
        ...
        mediaType        = mediaTypeFallback (mediaType, info, _prefs.autoStream);
        string newMedia;
        int frameMin = 0;
        bool hasSlate = true;
        float pa = 0.0;
        ...
        //  rangeStart, pixel aspect
        //

        let rs = int.max;
        if (frameMin != -int.max && mediaIsMovie (newMedia)) rs = frameMin - (if (hasSlate) then 1 else 0);
        ...
        try 
        {
            ...
            //
            //  Set media type, rangeStart (frame mapping), pixel aspect ratio.
            //
            _setMediaType (mediaType, sourceName);

            let rsProp = sourceName + ".group.rangeStart";
            if (rs != int.max)
            {
                if (!propertyExists (rsProp)) newProperty (rsProp, IntType, 1);
                setIntProperty (rsProp, int[] {rs}, true);
            }
            else
            {
                if (propertyExists (rsProp)) setIntProperty (rsProp, int[] {}, true);
            }

	    try
            {
                let sourceGroup = nodeGroup(sourceName),
                    linPipeline = extra_commands.nodesInGroupOfType (sourceGroup, "RVLinearizePipelineGroup")[0],
                    paNode      = extra_commands.nodesInGroupOfType (linPipeline, "RVLensWarp")[0],
            ...
            {
                deb ("    Failed to set PA on source %s\n" % sourceName);
            }
        }
        catch (object obj)
        {
    ...
    method: sessionFromEDL (void; string[] sources, string[] media, string[] mediaTypes, int[] rss, float[] pas, int[] ins, int[] outs)
    {
        deb ("sessionFromEDL called media %s rss %s pas %s ins %s outs %s\n" % (media, rss, pas, ins, outs));
    ...
    method: baseSessionFromInfos (void; string[] sources, int[] rangePrefs, StringMap[] infos)
    {
        ...
        for_index (i; infos)
        {
            ...
            //  
            //  Only build rangeStart, media, pixelAspect arrays if
            //  we are actually adding sources.
            //
            deb ("    _prefs.loadMedia %s\n" % _prefs.loadMedia);
            if (rangePrefs eq nil)
            {
                mediaType = mediaTypeFallback (_prefs.loadMedia, inf, _prefs.autoStream);
                ...
                {
                    edlRS.push_back (frameMin - (if (hasSlate) then 1 else 0));
                }
                else edlRS.push_back (int.max);
            }
        ...
        sessionFromEDL (sources, edlMedia, edlMediaTypes, edlRS, edlPAs, edlIns, edlOuts);
    ...
    method: sessionFromInfos (void; bool doCompare, bool clearFirst, StringMap[] infos)
    {
        ...
        try
        {
        if (clearFirst && sources().size() > 0) clearViews();
    ...
    {
        let serverReady = _shotgunState.serverReady();

        deb ("sessionFromVersionIDs called ids '%s', serverReady %s, _serverSwitchedWithToken %s\n" %
                (ids, serverReady, _serverSwitchedWithToken));

        if (!serverReady || _serverSwitchedWithToken)
        {
            _serverSwitchedWithToken = false;

            let contents = _shotgunState._serverURL;

            if (!serverReady) 
            {
                contents += "|rvlink:// -reuse 1 -play -l -eval 'shotgun.reconnectToServer(); shotgun.%sFromVersionIDs(%s);'" %
                         (if (doCompare) then "compare" else "session", ids);
            }

            deb ("sessionFromVersionIDs: server is not ready or switching servers, sending event\n");
            deb ("    contents '%s'\n" % contents);
            sendInternalEvent ("shotgun-credentials-missing", contents);
    ...
    */
    ...
    {
        deb ("INFO: retrieved %s Versions in %s seconds\n" % (infos.size(), (theTime() - startTime)));
        if (newSession)
        {
            int[] ids = int[]();
    ...
    method: allEntityVersions (void; string name, string fieldName, string fieldType, bool projConstraint, Event e)
    {
        ...
        if (nil eq info)
        {
            deb ("WARNING: source #%s has no tracking info\n" % source);
            return;
        }
    ...
    method: _buildCurrentSourceMenu (Menu; bool oldStyle )
    {
        let m1 = Menu(),
            types = string[]();
        ...
        if (shotgun_fields.initialized)
        {
            ...
            if (types.size() > 1)
            {
                ...
                for_each (t; types)
                {
                    m1.push_back (MenuItem {"    " + ( if(oldStyle) then "" else "Play " ) + t, swapMedia("one", t, ),
        ...
        Menu m2 = Menu {
            {"_", nil},
            { if(oldStyle) then "Edit Clip" else "Frame Range", nil, nil, disabledFunc},
    ...
    method: _buildPrefsMenu (Menu; )
    {
        ...
        Menu m4 = Menu {
            ...
            {"Enable Streaming", toggleStreaming, nil, autoStreaming},
        };
    ...
    method: _buildDefaultConnectionDetailsSubMenu(Menu; )
    {
        return Menu { { "_", nil }};
    }

    method: _buildMenu (Menu; Menu connectionDetailsSubMenu )
    {
        if ( connectionDetailsSubMenu eq nil ) {
            connectionDetailsSubMenu = _buildDefaultConnectionDetailsSubMenu();
        }

        Menu m5;
        (Menu; ShotgunMinorMode, Menu) fct = nil;

        try {
            runtime.name externalBuildMenuName = runtime.intern_name("shotgun_review_app.externalBuildMenu");
            fct = runtime.lookup_function(externalBuildMenuName);
        }
        catch (...) {;}
 
        if ( fct neq nil ) {
            // Let Screening Room build the menu 
            m5 = fct(this, connectionDetailsSubMenu);
        } else {
            Menu m1 = Menu {
            ...
            Menu m4 = Menu {
                ...
                //  {"Add Audio", doNothingEvent, nil, disabledFunc},
                {"_", nil},
                {"Current Source Only", _buildCurrentSourceMenu( /* old ? */ true )},
                {"_", nil},
    ...
    {
        let oldServer = _prefs.serverURL;
        deb("setServerURLValue '%s' oldServer '%s'\n" % (v, oldServer));
        if (v != "")
        {
            if (v == " ") v = "";
            _prefs.serverURL = v;
        ...
        let currentSession = slutils.getCurrentSessionParams();

        _serverSwitchedWithToken = (
            _shotgunState.serverReady() &&
            !_shotgunState.serverUsingScriptKey() &&
            (v != oldServer || v != currentSession._0));

        deb("setServerURLValue: server is nil %s\n" % (_shotgunState._shotgunServer eq nil));

        if (_shotgunState.serverReady())
        {
            _prefs.serverURL = _shotgunState._serverURL;
            _prefs.writePrefs();
            if (v != "" && v != oldServer) extra_commands.displayFeedback("New Server: " + v);
        }
        else deb("********************* setServerURLValue failed\n");
    }

    // obsolete: this function is not called anymore...
    /*
    ...
    */
    ...
    method: rebuildMenu(void; Event event)
    {
        if (event neq nil) event.reject();

        slmodule.SLModule slmode = slmodule.theMode();
        let connectionDetailsSubMenu =  Menu();

        if (slmode neq nil) connectionDetailsSubMenu =  slmode.getConnectionDetailsMenu();

        setMenuStrict(_buildMenu(connectionDetailsSubMenu));
    }
    ...
    method: findEntityInURL ((int, string); string url)
    {
        ...
        if (parts neq nil && parts.size() == 3)
        {
            ...
            if (parts[1] == "Element")   return (int(parts[2]), "Element");
            if (parts[1] == "Sequence")  return (int(parts[2]), "Sequence");
        }
        return (-1, string(nil));
    }
    ...
    method: urlDoDrop (void; int region, string url)
    {
        ...
        else if (etype == "Element")  entityVersions (id, "entity", "Element");
        else if (etype == "Sequence") entityVersions (id, "entity", "Sequence");
    ...
    method: ShotgunMinorMode (ShotgunMinorMode;)
    {
        ...
        // if the slmodule module already exists we need to call it to get the connection details submenu
        // otherwise we are doing this for nothing but it's harmless
        // (with race conditions we never know who will be called first...)
        ...
        init("Shotgun",
             ...
             [("frame-changed", frameChanged, "Update frame"),
              ...
              ("session-initialized", rebuildMenu, "Session initialize - rebuildMenu"),
              ("after-progressive-loading", afterProgressiveLoading, "Update Infos After Load")],
             _buildMenu(connectionDetailsSubMenu));
    ...
    method: render (void; Event event)
    {
        ...
        glyph.setupProjection(w, h, event.domainVerticalFlip());
            ...
            string   eventKey)
    {
        ...
        let key   = _shotgunState._scriptKey,
            token = _shotgunState._token;

        if (token neq nil && token != "")
        {
            contents += param ("session_token", token);
        }
        else
        {
            contents += param ("script_name", "rv");
            contents += param ("script_key",  key);
        }
        ...
        function: authHandler (void; Event event) { deb("auth: %s\n" %  event.contents()); }
        function: errorHandler (void; Event event) { deb("progress: %s\n" %  event.contents()); }
        function: progressHandler (void; Event event) { deb("error: %s\n" %  event.contents()); }
...
\: _combineMenus (Menu; Menu a, Menu b)
{
...
{
    ShotgunMinorMode m = localMode;

    if (m eq nil) 
    {
        print ("--------------------------------- shotgun localMode is nil!\n");
        m = rvui.minorModeFromName("Shotgun");
    }

    return m;
}

\: externalBuildMenu(Menu; Menu submenu)
{
    ShotgunMinorMode m = theMode();

    Menu ret = nil;
    if (m neq nil) ret = m._buildMenu(submenu);
...
\: reconnectToServer(void;)
{
    ...
    {
        ShotgunMinorMode m = theMode();
        m._shotgunState.connectToServer();
    }
    catch (object obj)
    {
        print ("ERROR: shotgun_mode.reconnectToServer() failed: %s" % string(obj));
    }
}
...
\: sessionFromVersionIDs(string; int[] ids, string flags="")
{
    try
    {
        clearViews();
        ShotgunMinorMode m = theMode();
        m.sessionFromVersionIDs (ids, false, false);
    }
    catch (object obj)
    {
...
\: compareFromVersionIDs(string; int[] ids, string flags="")
{
    try
    {
        ...
        ShotgunMinorMode m = theMode();
        m.sessionFromVersionIDs (ids, true, false);
    }
    catch (object obj)
    {
