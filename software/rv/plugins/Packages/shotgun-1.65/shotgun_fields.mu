...
function:_initFieldsConfig (void; string configStyle)
{
    ...
    if (system.getenv("RV_SHOTGUN_IGNORE_CUSTOM_CONFIG", nil) neq nil)
    {
        mName = "shotgun_fields_config_standard";
        if (!runtime.load_module(mName)) 
        {
            print ("ERROR: Ignoring custom config, but '%s' module not found.\n" % mName);
            mName = "";
        }
    }
    else
    {
        if (configStyle != "")
        {
            ...
            if (!runtime.load_module(mName)) 
            {
                print ("INFO: Requested custom config style '%s' not found\n" % configStyle);
                mName = "";
            }
        }
        if (mName == "")
        {
            ...
            if (!runtime.load_module(mName)) 
            {
                ...
                if (!runtime.load_module(mName)) 
                {
                    print ("ERROR: '%s' module not found (and no custom module).\n" % mName);
                    mName = "";
...
function: getStreamingURL (string; StringMap info)
{
    string s = "%s/file_serve/version/%d/mp4" % ( info.findString("shotgunURL"), info.findInt("id") );
    return s;
}
...
function: mediaTypePath (string; string mediaType, StringMap info)
{
    ...
    {
        deb ("Streaming with %s\n" % ( getStreamingURL(info) ) );
        return getStreamingURL(info);
    }
...
function: mediaTypePixelAspect (float; string mediaType, StringMap info)
{
    ...
    if (pas eq nil)
    {
        print ("WARNING: %s not set, relying on file metadata\n" % name);
        pas = "0.0";
    }
    return float(pas);
}
...
function: mediaTypeHasSlate (bool; string mediaType, StringMap info)
{
    ...
    if (hss eq nil)
    {
        print ("WARNING: %s not set, assuming false\n" % name);
        hss = "false";
    }
    return bool(hss);
}
