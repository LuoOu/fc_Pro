...
function: parseResult (Value; string xml, string url)
{
    use Value;
    ...
    */
    ...
    for_index (i; lines)
    {
        ...
        if (line == "<fault>") 
        {
            ...
            if (code == 102)
            {
                let serverURL = regex.replace("/api3_preview/", url, "");
                commands.sendInternalEvent ("shotgun-credentials-invalid", string(code) + "|" + serverURL);
            }
            else print("ERROR: XML-RPC: faultCode=%d -- %s\n" % (code, why));
            break;
        }
                ...
                ReturnFunc rvalFunc)
{
    ...
    \: returnFunc (void; Event event)
    {
        ...
        try
        {
            v = parseResult(event.contents(), url); 
        }
        catch (exception exc)
        {
...
\: test (void;)
{
    ...
    */
    let p1   = Struct( [ ("session_token", String("33409d2bf11cbcbc5d534ddcfb0c9707")) ] );
