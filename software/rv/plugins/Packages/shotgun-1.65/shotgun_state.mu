...
function: lookupServer((string, string, string); string defaultURL)
{
    ...
    (string, string, string) nilTuple;
    nilTuple._0 = nilString;
    nilTuple._1 = nilString;
    nilTuple._2 = nilString;
    ...
    deb("lookupServer: serverMap: '%s', defaultURL '%s'\n" % (serverMap, defaultURL));

    //
    //  Try script_key first, since requires less user interaction.
    //
    if (serverMap != "")
    {
        ...
        {
            if (parts[i] == defaultURL) return (defaultURL, parts[i+1], nilString);
        }
    }
    deb("lookupServer: script_key not found\n");

    require slutils;
    let token = slutils.getSessionIdFromUrl(defaultURL);

    if (token == "") token = nilString;

    deb("lookupServer: token '%s'\n" % token);

    return (defaultURL, nilString, token);
}
...
class: ShotgunState
{
    ...
    method: serverReady(bool;)
    {
        return (_shotgunServer neq nil);
    }

    method: serverUsingScriptKey(bool;)
    {
        return (_scriptKey neq nil && _token eq nil);
    }
    ...
    method: connectToServer(void; )
    {
        try 
        {
            ...
            if ( _serverURL eq nil || _serverURL == "" ) {
                // this happens on the first launch of RV after authenticating with 
                return;
            }

            let (url, scriptKey, token) = lookupServer (_serverURL);
            ...
            if (_scriptKey neq nil || _token neq nil)
            {
