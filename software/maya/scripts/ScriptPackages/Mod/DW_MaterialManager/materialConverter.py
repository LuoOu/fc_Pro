...
try:
    ...
    if not os.path.exists(userPath) == True:
        os.mkdir(userPath)
    if not os.path.exists(userPath + "/materialAttrs.json") == True:
        shutil.copy(scriptPath + "/materialAttrs.json", userPath)
    if not os.path.exists(userPath + "/bumpAttrs.json") == True:
        shutil.copy(scriptPath + "/bumpAttrs.json", userPath)
    if not os.path.exists(userPath + "/subdiv_displacementAttrs.json") == True:
        shutil.copy(scriptPath + "/subdiv_displacementAttrs.json", userPath)
except:
    pass
...
def materialConvert(sourceMaterialType,targetMaterialType,sourceColorSpace,targetColorSpace,affectColorSwitch,*args):
    sels = mc.ls(sl = 1)
    dataDict = prepareData(['materialAttrs','subdiv_displacementAttrs','bumpAttrs'])
    materialData = dataDict['materialAttrs']
